(function attachSignupFlowHelpers(root, factory) {
  ...
  function createSignupFlowHelpers(deps = {}) {
    ...
    function isSignupContactVerificationPageUrl(rawUrl) {
      const parsed = parseUrlSafely(rawUrl);
      if (!parsed) return false;
      return /\/contact-verification(?:[/?#]|$)/i.test(parsed.pathname || '');
    }
    ...
    function resolveSignupPostIdentityState(rawUrl, options = {}) {
      if (isSignupPasswordPageUrl(rawUrl)) {
        return 'password_page';
      }
      if (isSignupContactVerificationPageUrl(rawUrl)) {
        return options.contactVerificationState || 'verification_page';
      }
    ...
    async function confirmSignupPasswordPageByContentScript(tabId, step = 2, options = {}) {
      ...
      try {
        await ensureContentScriptReadyOnTab('signup-page', tabId, {
          ...
          timeoutMs: 45000,
          retryDelayMs: 900,
          logMessage: `步骤 ${step}：URL 未命中密码页，正在通过页面内容确认是否已进入密码页...`,
        });
        ...
        const result = await sendToContentScriptResilient('signup-page', {
          ...
          timeoutMs: 15000,
          retryDelayMs: 700,
          logMessage: `步骤 ${step}：正在通过密码输入框确认当前页面是否已进入密码页...`,
        });
        ...
        if (result?.ready || result?.state === 'password_page') {
          return {
            ...(result || {}),
            ready: true,
            state: 'password_page',
        ...
        // The normal URL-based error below is clearer for the caller.
      }
      return null;
    }
    ...
    async function ensureSignupPostIdentityPageReadyInTab(tabId, step = 2, options = {}) {
      ...
      if (!skipUrlWait) {
        const matchedTab = await waitForTabUrlMatch(tabId, (url) => Boolean(resolveSignupPostIdentityState(url, options)), {
          timeoutMs: 45000,
          retryDelayMs: 300,
        });
        if (!matchedTab) {
          if (typeof addLog === 'function') {
            await addLog(
              `步骤 ${step}：等待注册身份提交后的 URL 跳转超时，改为检查当前页面内容是否已进入密码页。`,
              'warn'
            );
          }
        } else {
          landingUrl = matchedTab.url || '';
          landingState = resolveSignupPostIdentityState(landingUrl, options);
        }
      }
      ...
      if (!landingState) {
        const passwordPageResult = await confirmSignupPasswordPageByContentScript(tabId, step, {
          fallbackUrl: landingUrl,
        });
        if (passwordPageResult) {
          return passwordPageResult;
        }
      ...
      if (landingState !== 'password_page' && typeof waitForTabStableComplete === 'function') {
        ...
        if (stableTab?.url) {
          const stableState = resolveSignupPostIdentityState(stableTab.url, options);
          if (stableState) {
