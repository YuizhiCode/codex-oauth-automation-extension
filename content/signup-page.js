...
function isEmailVerificationPage() {
  return /\/(?:email|contact)-verification(?:[/?#]|$)/i.test(location.pathname || '');
}
...
function getSignupCountryLabelAliases(value) {
  ...
  const compact = normalizeSignupCountryLabel(raw).replace(/\s+/g, '');
  if (
    ...
    ['GB', 'UK', 'United Kingdom', 'Great Britain', 'Britain', 'England', '英国', '英格兰', '大不列颠'].forEach(addAlias);
  }
...
function isSignupPhoneCountrySelectionSynced(phoneInput, targetOption, options = {}) {
  const targetDialCode = resolveSignupPhoneTargetDialCode(options, targetOption);
  const countryButton = getSignupPhoneCountryButton(phoneInput);
  ...
  if (countryButton && isVisibleElement(countryButton)) {
    return false;
  }
...
function isLikelyLoggedInChatgptHomeUrl(rawUrl = location.href) {
  ...
  try {
    ...
    if (/^\/(?:auth\/|create-account\/|email-verification|contact-verification|log-in|add-phone)(?:[/?#]|$)/i.test(path)) {
      return false;
    }
...
function isVerificationPageStillVisible() {
  ...
  if (document.querySelector('form[action*="email-verification" i], form[action*="contact-verification" i]')) return true;
...
function isAddPhonePageReady() {
  ...
  if (typeof getLoginPasswordInput === 'function' && getLoginPasswordInput()) {
    return false;
  }
...
function getAddEmailInput() {
  const input = document.querySelector(
    'input[type="email"], input[name="email"], input[name="username"], input[id*="email"], input[placeholder*="email" i], input[placeholder*="Email"], input[aria-label*="email" i], input[aria-label*="Email"]'
  );
...
function getSignupAuthRetryPathPatterns() {
  return [
    /\/(?:create-account|log-in)\/password(?:[/?#]|$)/i,
    /\/(?:email|contact)-verification(?:[/?#]|$)/i,
  ];
}
...
async function waitForLoginVerificationPageReady(timeout = 10000, visibleStep = 8, options = {}) {
  ...
  throw new Error(
    `当前未进入登录验证码页面，请先重新完成步骤 ${getAuthLoginStepForLoginCodeStep(visibleStep)}。当前状态：${getLoginAuthStateLabel(snapshot)}。URL: ${snapshot?.url || location.href}`
  );
}
...
function createStep6AddEmailSuccessResult(snapshot, options = {}) {
  const result = createStep6SuccessResult(snapshot, {
    ...options,
    via: options.via || 'add_email_page',
    loginVerificationRequestedAt: null,
  });
  result.addEmailPage = true;
  return result;
}
...
async function waitForSignupVerificationTransition(timeout = 5000, options = {}) {
  const start = Date.now();
  const settleFullDuration = Boolean(options?.settleFullDuration);
  let snapshot = inspectSignupVerificationState();

  while (Date.now() - start < timeout) {
    throwIfStopped();

    snapshot = inspectSignupVerificationState();
    if (
      !settleFullDuration
      && (
    ...
    const remainingMs = timeout - (Date.now() - start);
    await sleep(Math.max(0, Math.min(200, remainingMs)));
  }
...
async function step6LoginFromPasswordPage(payload, snapshot) {
  ...
  if (currentSnapshot.passwordInput) {
    ...
    if (transition.action === 'done') {
      if (transition.result?.addEmailPage) {
        return completeAddEmailDuringStep7(payload, {
          via: transition.result.via || 'password_submit_add_email',
        });
      }
...
async function submitAddEmailAndContinue(payload = {}) {
  const visibleStep = Math.floor(Number(payload?.visibleStep) || 0) || 8;
  const email = String(payload?.email || '').trim();
  if (!email) {
    throw new Error('添加邮箱时缺少邮箱地址。');
  }

  const currentSnapshot = normalizeStep6Snapshot(inspectLoginAuthState());
  if (currentSnapshot.state === 'verification_page') {
    return {
      success: true,
      alreadyOnVerificationPage: true,
      displayedEmail: getLoginVerificationDisplayedEmail() || email,
      url: location.href,
    };
  }
  if (currentSnapshot.state === 'oauth_consent_page') {
    return {
      success: true,
      directOAuthConsentPage: true,
      url: location.href,
  ...
  if (currentSnapshot.state !== 'add_email_page' && !isAddEmailPageReady()) {
    throw new Error(`当前未进入添加邮箱页，无法添加邮箱。当前状态：${getLoginAuthStateLabel(currentSnapshot)}。URL: ${location.href}`);
  }

  const emailInput = getAddEmailInput() || currentSnapshot.emailInput || getLoginEmailInput();
  if (!emailInput) {
    throw new Error('在添加邮箱页未找到邮箱输入框。URL: ' + location.href);
  }

  await humanPause(500, 1400);
  fillInput(emailInput, email);
  log(`步骤 ${visibleStep}：已填写需要绑定的邮箱 ${email}`, 'info', {
    step: visibleStep,
    stepKey: visibleStep === 7 ? 'oauth-login' : 'fetch-login-code',
  });

  await sleep(500);
  const submittedAt = Date.now();
  await triggerLoginSubmitAction(currentSnapshot.submitButton || getLoginSubmitButton({ allowDisabled: true }), emailInput);
  log(`步骤 ${visibleStep}：已提交绑定邮箱，正在等待邮箱验证码页...`, 'info', {
    step: visibleStep,
    stepKey: visibleStep === 7 ? 'oauth-login' : 'fetch-login-code',
  });
  ...
  const transition = await waitForStep6PostSubmitTransition({
    timeout: 15000,
    visibleStep,
    via: 'add_email_submit',
    oauthConsentVia: 'add_email_submit_oauth_consent',
    loginVerificationRequestedAt: submittedAt,
    timeoutRecoveryMessage: '提交绑定邮箱后进入登录超时报错页。',
    timeoutRecoveryVia: 'add_email_submit_timeout_recovered',
    allowPhoneVerificationPage: true,
    stalledReason: 'add_email_submit_stalled',
    stalledMessage: '提交绑定邮箱后仍未进入邮箱验证码页。',
    addPhoneMessage: (snapshot) => `提交绑定邮箱后页面进入手机号页面。URL: ${snapshot.url}`,
  });
  ...
  if (transition.action === 'done') {
    return {
      success: true,
      displayedEmail: getLoginVerificationDisplayedEmail() || email,
      directOAuthConsentPage: Boolean(transition.result?.directOAuthConsentPage),
      loginVerificationRequestedAt: submittedAt,
      url: transition.result?.url || location.href,
    };
  }
  if (transition.action === 'recoverable') {
    throw new Error(transition.result?.message || '提交绑定邮箱后未进入邮箱验证码页。');
  }

  throw new Error(`提交绑定邮箱后未得到可用状态。当前状态：${getLoginAuthStateLabel(transition.snapshot)}。`);
}
...
async function completeAddEmailDuringStep7(payload = {}, options = {}) {
  ...
  const email = String(payload?.email || '').trim();
  if (!email) {
    return createStep6RecoverableResult('missing_add_email_address', normalizeStep6Snapshot(inspectLoginAuthState()), {
      message: '手机号登录后进入绑定邮箱页，但当前步骤没有可填写的邮箱地址。',
    });
  }

  const result = await submitAddEmailAndContinue({
    ...payload,
    email,
    visibleStep,
  });
  const snapshot = normalizeStep6Snapshot(inspectLoginAuthState());
  if (result?.directOAuthConsentPage || snapshot.state === 'oauth_consent_page') {
    return createStep6OAuthConsentSuccessResult(snapshot, {
      via: `${options.via || 'add_email_submit'}_oauth_consent`,
    });
  }
  return createStep6SuccessResult({
    ...snapshot,
    state: 'verification_page',
    displayedEmail: result?.displayedEmail || email,
    url: result?.url || snapshot.url || location.href,
  }, {
    via: options.via || 'add_email_submit',
...
function getLoginPhoneInputCandidateDiagnostics(limit = 12) {
  return Array.from(document.querySelectorAll('input'))
    .filter((input) => input)
    .slice(0, limit)
    .map((input) => summarizePhoneInputCandidate(input, {
      allowGenericText: isLoginPhoneUsernameKind() || isLoginPhoneEntryPageText(),
    }));
}

function getLoginPhoneDisplayedDialCode(phoneInput = null) {
  const signupDisplayedDialCode = getSignupPhoneDisplayedDialCode(phoneInput);
  if (signupDisplayedDialCode) {
    return signupDisplayedDialCode;
  }

  const inputRoot = phoneInput?.closest?.('fieldset, form, [data-rac], div') || document;
  const rootText = String(inputRoot?.textContent || '').replace(/\s+/g, ' ').trim();
  const rootDialCode = extractDialCodeFromText(rootText);
  if (rootDialCode) {
    return rootDialCode;
  }
...
function resolveLoginPhoneDialCode(phoneInput, options = {}) {
  const { phoneNumber = '', countryLabel = '' } = options;
  const signupDialCode = resolveSignupPhoneDialCode(phoneInput, options);
  if (signupDialCode) {
    return signupDialCode;
  }

  return resolveSignupPhoneDialCodeFromNumber(phoneNumber, [
    countryLabel,
    getLoginPhoneDisplayedDialCode(phoneInput),
    getPageTextSnapshot(),
  ]);
}
...
async function step6LoginFromPhonePage(payload, snapshot) {
  const visibleStep = Math.floor(Number(payload?.visibleStep) || 0) || 7;
  const currentSnapshot = normalizeStep6Snapshot(snapshot || inspectLoginAuthState());
  const phoneInput = currentSnapshot.phoneInput || Array.from(document.querySelectorAll('input')).find((input) => {
    if (!input || !isVisibleElement(input)) return false;
    const type = String(input.getAttribute?.('type') || input.type || '').trim().toLowerCase();
    const name = String(input.getAttribute?.('name') || input.name || '').trim().toLowerCase();
    const id = String(input.getAttribute?.('id') || input.id || '').trim().toLowerCase();
    const placeholder = String(input.getAttribute?.('placeholder') || '').trim();
    const ariaLabel = String(input.getAttribute?.('aria-label') || '').trim();
    return (
      type === 'tel'
      || /phone|tel|username/i.test(`${name} ${id}`)
      || /手机|电话|手机号|电话号码|phone|mobile|telephone/i.test(`${placeholder} ${ariaLabel}`)
    ) && Number(input.getAttribute?.('maxlength') || input.maxLength || 0) !== 6;
  });
  ...
  const dialCode = await selectCountryForPhoneInput(phoneInput, phoneNumber, countryLabel, {
    countryId,
    visibleStep,
  }) || resolveLoginPhoneDialCode(phoneInput, {
    phoneNumber,
    countryId,
    countryLabel,
  });
  const activePhoneInput = getLoginPhoneInput() || phoneInput;
  const inputValue = toNationalPhoneNumber(phoneNumber, dialCode);
  if (!inputValue) {
    throw new Error(`步骤 ${visibleStep}：手机号为空，无法填写。`);
  }
  ...
  log(
    `步骤 ${visibleStep}：手机号登录填写前诊断 ${JSON.stringify({
      ...
      dialCode,
      input: {
        tag: (activePhoneInput.tagName || '').toLowerCase(),
        type: String(activePhoneInput.getAttribute?.('type') || activePhoneInput.type || '').trim().toLowerCase(),
        name: String(activePhoneInput.getAttribute?.('name') || activePhoneInput.name || '').trim(),
        id: String(activePhoneInput.getAttribute?.('id') || activePhoneInput.id || '').trim(),
        value: getPhoneInputRenderedValue(activePhoneInput),
      },
  ...
  await humanPause(500, 1400);
  const fillResult = await fillLoginPhoneInputAndConfirm(activePhoneInput, {
    phoneNumber,
    dialCode,
    visibleStep,
    resolvePhoneInput: () => getLoginPhoneInput() || activePhoneInput,
  });
  ...
  await sleep(500);
  const verifiedPhoneInput = fillResult.input || activePhoneInput;
  ...
  const preSubmitDiagnostics = {
    ...
    hidden: {
      tag: (preSubmitHiddenInput?.tagName || '').toLowerCase(),
      type: String(preSubmitHiddenInput?.getAttribute?.('type') || preSubmitHiddenInput?.type || '').trim().toLowerCase(),
      name: String(preSubmitHiddenInput?.getAttribute?.('name') || preSubmitHiddenInput?.name || '').trim(),
      id: String(preSubmitHiddenInput?.getAttribute?.('id') || preSubmitHiddenInput?.id || '').trim(),
      value: String(preSubmitHiddenInput?.value || preSubmitHiddenInput?.getAttribute?.('value') || '').trim(),
    },
  ...
  const transition = await waitForStep6PostSubmitTransition({
    ...
    allowPhoneVerificationPage: true,
    allowPasswordAction: true,
    allowPhoneAction: true,
    allowFinalPhoneAction: true,
    stalledReason: 'phone_submit_stalled',
    stalledMessage: '提交手机号后长时间未进入密码页或手机验证码页。',
    addPhoneMessage: (nextSnapshot) => `提交手机号后页面直接进入手机号补全页面，未经过登录验证码页。URL: ${nextSnapshot.url}`,
  });
  if (transition.action === 'done') {
    if (transition.result?.skipLoginVerificationStep || transition.result?.addEmailPage) {
      return transition.result;
...
async function step6_login(payload) {
  ...
  const loginIdentifierType = String(payload?.loginIdentifierType || '').trim().toLowerCase();
  ...
  if (snapshot.state === 'add_email_page') {
    log('认证页已在添加邮箱页，正在绑定邮箱并进入邮箱验证码页。', 'info', { step: visibleStep, stepKey: 'oauth-login' });
    return completeAddEmailDuringStep7(payload, {
